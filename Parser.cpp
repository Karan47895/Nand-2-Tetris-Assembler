...
Parser::~Parser() {
    compDict.clear();  // delete all content in symbol table
    destDict.clear();
    jumpDict.clear();
}
...
string Parser::parseAInst(string inst) {
    ...
    // else if(inst[0]=='R'){ 
    //   string used = inst.substr(1,inst.size());
    //   return prefixA + bitset<15>(stoi(inst)).to_string();
    //
    else {
        // number in front of letter?
        if (inst.find_first_of("0123456789") < inst.find_first_not_of("0123456789")) {  // invalid
