...
int main(int argc, char const *argv[]) {
    ...
    if (argc < 2) {
        ...
        if (argc == 2) {
            int idx = fnameIn.find_last_of('.');  // find the base name
        ...
        /* Pre pass: read source file clean-up the code */
               ...
               while (getline(fIn, line)) {
                   /* remove comments */
                   int idx = line.find("//");  // find start of "//"
                   string lineRmComm = line.substr(0, idx);
                   if (lineRmComm.empty()) continue;  // skip empty line
       
                   /* remove leading spaces and tabs */
                   lineRmComm.erase(0, lineRmComm.find_first_not_of(" \t"));
       
                   /* remove spaces inside the instruction */
                   lineRmComm.erase(remove(lineRmComm.begin(), lineRmComm.end(), ' '), lineRmComm.end());
                   lineRmComm.erase(remove(lineRmComm.begin(), lineRmComm.end(), '\t'), lineRmComm.end());
       
                   if (!lineRmComm.empty()) {
                       queue1.push(lineRmComm);  // push to queue
                       cout << lineRmComm << endl;
                   }
               }
